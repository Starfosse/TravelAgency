export const PRODUCT_CATEGORIES = [
  {
    label: "Europe",
    value: "europe" as const,
    featured: [
      {
        name: "Venise",
        slug: "venise",
        price: "200",
        description: `🎭 Explorez la magie de Venise : Un séjour inoubliable vous attend ! 🎭

        Plongez dans l'enchantement de la Sérénissime lors d'un voyage exquis à Venise, où chaque coin de rue vous réserve une nouvelle surprise et chaque pont vous emporte dans un conte de fées.
        
        🏰 Hébergement dans des palais historiques : Imprégnez-vous de l'atmosphère opulente de Venise en séjournant dans des hôtels prestigieux situés le long du Grand Canal. Des palais magnifiquement restaurés vous accueillent dans un luxe digne des doges, offrant une expérience inoubliable.
        
        🚣 Promenade en gondole : Glissez sur les eaux miroitantes des canaux vénitiens à bord d'une gondole traditionnelle, tandis que votre gondolier expert vous guide à travers les dédales romantiques de la ville. Laissez-vous enivrer par le charme incomparable de Venise, avec ses palais qui semblent flotter sur l'eau et ses ponts ornés.
        
        🏛️ Visite des trésors historiques : Explorez les joyaux architecturaux de Venise, de la basilique Saint-Marc avec ses mosaïques étincelantes au palais des Doges orné de chefs-d'œuvre artistiques. Plongez dans l'histoire fascinante de la ville, où chaque pierre raconte une histoire millénaire.
        
        🛶 Excursions dans les îles de la lagune : Partez à la découverte des trésors cachés de la lagune vénitienne en visitant les îles pittoresques de Murano, Burano et Torcello. Admirez l'artisanat du verre à Murano, les maisons colorées de Burano et la tranquillité sereine de Torcello, chacune offrant une expérience unique.
        
        🍽️ Gastronomie divine : Savourez les délices culinaires de Venise dans des trattorias intimes et des restaurants raffinés. Dégustez des plats de fruits de mer fraîchement pêchés, des cicchetti savoureux et des pâtisseries exquises, accompagnés de vins locaux envoûtants.
        
        ✨ Magie et romantisme à chaque instant : Perdez-vous dans les ruelles étroites de Venise, où chaque coin révèle une nouvelle surprise et chaque coucher de soleil peint un tableau de rêve. Laissez-vous emporter par la romance et la beauté intemporelle de cette ville unique au monde.
        
        Votre voyage à Venise promet d'être une expérience inoubliable, où chaque moment est une découverte et chaque instant est une merveille. Réservez dès maintenant et préparez-vous à vivre des souvenirs qui dureront toute une vie dans la ville des amoureux et des rêveurs.`,
        imageSrc: "/nav/europe/Venise.jpg",
      },
      {
        name: "Barcelone",
        slug: "barcelone",
        price: "200",
        description: `🌞🏖️ Explorez la vibrante Barcelone : Un séjour ensoleillé entre culture et détente ! 🏰🍷

        Bienvenue dans la ville enchanteresse de Barcelone, où l'art, la culture, la gastronomie et les plages dorées se rencontrent pour créer une expérience de voyage incomparable.
        
        🏨 Hébergement dans des hôtels de charme : Découvrez le confort et l'élégance en séjournant dans des hôtels situés au cœur de la ville ou en bord de mer, offrant une vue imprenable sur les attractions emblématiques et les plages scintillantes de Barcelone.
        
        🎨🏰 Exploration des trésors architecturaux : Parcourez les rues sinueuses de Barcelone et admirez les chefs-d'œuvre architecturaux uniques de Gaudí, tels que la Sagrada Família, le Parc Güell et la Casa Batlló. Plongez-vous dans l'histoire fascinante de la ville en visitant le quartier gothique avec ses ruelles médiévales et ses bâtiments historiques.
        
        🍷🍽️ Découverte de la gastronomie catalane : Savourez les délices de la cuisine méditerranéenne dans les tavernes traditionnelles, les bars à tapas animés et les restaurants étoilés. Goûtez aux spécialités locales telles que la paella, le jamón ibérico et les délicieux fruits de mer frais, accompagnés d'un verre de vin espagnol rafraîchissant.
        
        🏖️🌴 Farniente sur les plages de sable fin : Détendez-vous et profitez du soleil méditerranéen sur les plages pittoresques de Barcelone, telles que la Barceloneta, la plage de Bogatell ou la plage de Mar Bella. Rafraîchissez-vous dans les eaux cristallines de la mer et savourez l'atmosphère animée des clubs de plage et des bars en bord de mer.
        
        🎭🎶 Vie nocturne animée : Vibrez au rythme de la vie nocturne effervescente de Barcelone, avec ses bars branchés, ses clubs animés et ses soirées festives qui durent jusqu'au petit matin. Découvrez la scène musicale dynamique de la ville, des concerts de flamenco aux DJ sets électro.
        
        🛍️🛒 Shopping et artisanat local : Parcourez les boutiques élégantes, les marchés colorés et les boutiques d'artisanat local de Barcelone pour dénicher des trésors uniques, des souvenirs authentiques et des produits artisanaux de qualité.
        
        Votre séjour à Barcelone promet d'être une expérience inoubliable, où la richesse culturelle et la beauté naturelle se marient harmonieusement pour créer des souvenirs qui dureront toute une vie. Réservez dès maintenant et laissez-vous envoûter par le charme envoûtant de cette ville méditerranéenne unique ! 🇪🇸✨`,
        imageSrc: "/nav/europe/Barcelone.jpg",
      },
      {
        name: "Munich",
        slug: "munich",
        price: "200",
        description: `🥨🍻 Découvrez l'essence de Munich : Un séjour bavarois inoubliable ! 🏰🌳

        Plongez-vous dans l'atmosphère enivrante de Munich lors d'un séjour captivant dans cette ville bavaroise riche en histoire, en culture et en traditions vibrantes.
        
        🏰 Hébergement dans des hôtels de charme : Imprégnez-vous du charme bavarois en séjournant dans des hôtels confortables et accueillants, situés à proximité des sites emblématiques de la ville. Des établissements traditionnels aux hébergements modernes, Munich offre une gamme d'options pour tous les goûts et tous les budgets.
        
        🥨🍻 Dégustation de la bière et des spécialités locales : Plongez dans l'ambiance animée des biergartens traditionnels, où vous pourrez déguster une délicieuse bière allemande accompagnée de pretzels moelleux et d'autres spécialités culinaires bavaroises. Ne manquez pas de visiter la célèbre Hofbräuhaus pour une expérience authentique de la culture brassicole munichoise.
        
        🏰 Exploration des trésors historiques : Parcourez les rues pavées de Munich et découvrez son riche patrimoine historique, de la majestueuse Marienplatz avec son imposant hôtel de ville aux somptueux palais royaux comme le château de Nymphenburg. Admirez l'architecture magnifique et plongez dans l'histoire fascinante de la ville.
        
        🌳🚴 Promenades dans les parcs et les jardins : Profitez de la beauté naturelle de Munich en vous promenant dans ses vastes parcs et jardins, tels que le parc Englischer Garten où vous pourrez vous détendre au bord de la rivière Isar ou louer un vélo pour explorer ses sentiers pittoresques.
        
        🎭🎶 Culture et divertissement : Assistez à des spectacles de musique et de théâtre dans les salles de concert renommées de Munich, ou découvrez les musées fascinants qui abritent des collections d'art et d'histoire captivantes. De l'opéra à la scène alternative, Munich offre une vie culturelle dynamique pour tous les goûts.
        
        🛍️🛒 Shopping et artisanat local : Flânez dans les rues commerçantes animées de Munich pour découvrir des boutiques uniques, des marchés traditionnels et des artisans locaux proposant des souvenirs uniques et des produits artisanaux de qualité.
        
        Votre séjour à Munich promet d'être une expérience enrichissante, où la convivialité bavaroise se mêle à la richesse culturelle de cette ville dynamique. Réservez dès maintenant et préparez-vous à vivre des moments mémorables au cœur de la Bavière ! 🇩🇪✨`,
        imageSrc: "/nav/europe/Munich.jpg",
      },
    ],
  },
  {
    label: "Asie",
    value: "asie" as const,
    featured: [
      {
        name: "Pekin",
        slug: "pekin",
        price: "200",
        description: `🏯🐉 Explorez la grandeur de Pékin : Un voyage épique dans l'ancienne capitale impériale de la Chine ! 🍜🏮

        Bienvenue dans la ville millénaire de Pékin, où l'histoire impériale, la culture vibrante et la modernité se rencontrent pour créer une expérience de voyage incomparable. Plongez dans l'essence même de la Chine ancienne et contemporaine lors de votre séjour dans cette métropole majestueuse.
        
        🏨 Hébergement dans des hôtels élégants et luxueux : Découvrez le confort et le raffinement en séjournant dans des hôtels prestigieux situés au cœur de la ville, offrant un mélange parfait de tradition chinoise et de commodités modernes. Des établissements de renommée mondiale vous accueillent dans un cadre somptueux, où chaque détail est pensé pour votre confort.
        
        🏯🎎 Exploration des trésors historiques et culturels : Parcourez les siècles d'histoire chinoise en visitant des sites emblématiques tels que la Cité interdite, le Temple du Ciel et la Grande Muraille de Chine, qui témoignent de la grandeur et de la richesse culturelle de l'ancienne dynastie impériale. Plongez-vous dans l'atmosphère mystique des hutongs traditionnels, où vous pourrez découvrir la vie quotidienne des habitants de Pékin.
        
        🍜🥟 Dégustation de la délicieuse cuisine chinoise : Éveillez vos papilles avec une incroyable variété de plats chinois, des nouilles sautées aux dim sum délicatement préparés, en passant par les canard laqué croustillant et les délices sucrés des desserts traditionnels. Explorez les marchés alimentaires animés et les restaurants locaux pour une expérience culinaire authentique.
        
        🌸🏞️ Émerveillement devant la beauté naturelle et les jardins chinois : Échappez à l'agitation de la ville en vous promenant dans les magnifiques jardins impériaux de Pékin, tels que le Jardin botanique de Pékin et le Jardin de Beihai, où vous pourrez profiter de la tranquillité et de la sérénité de la nature luxuriante.
        
        🎭🎶 Divertissements et spectacles traditionnels : Imprégnez-vous de la culture chinoise en assistant à des spectacles de théâtre traditionnel, des opéras de Pékin et des performances d'arts martiaux. Découvrez la scène artistique dynamique de la ville, des galeries d'art contemporain aux soirées de musique live.
        
        🛍️🎁 Shopping et artisanat local : Faites du shopping dans les marchés animés, les centres commerciaux élégants et les boutiques d'artisanat local de Pékin, où vous trouverez des trésors uniques, des souvenirs authentiques et des produits artisanaux traditionnels à ramener chez vous.
        
        Votre séjour à Pékin promet d'être une expérience inoubliable, où chaque moment est une découverte et chaque expérience est une merveille. Réservez dès maintenant et préparez-vous à vivre des moments magiques dans cette ville impériale légendaire de la Chine ! 🇨🇳✨`,
        imageSrc: "/nav/asie/pekin.jpg",
      },
      {
        name: "Tokyo",
        slug: "tokyo",
        price: "200",
        description: `🏙️🎌 Explorez l'émerveillement de Tokyo : Un voyage inoubliable au cœur du Japon ! 🍣🎎

        Bienvenue dans la métropole la plus dynamique du monde, où l'ancien et le moderne se rencontrent dans une symphonie de culture, de technologie et de traditions fascinantes. Tokyo, la ville aux mille visages, vous invite à découvrir ses merveilles uniques à chaque coin de rue.
        
        🏨 Hébergement dans des hôtels de classe mondiale : Plongez-vous dans le luxe et le confort en séjournant dans des hôtels prestigieux situés au cœur des quartiers animés de Tokyo. Des établissements de renommée mondiale vous offrent une hospitalité exceptionnelle et des vues imprenables sur la ville qui ne dort jamais.
        
        🏯🌸 Exploration des trésors historiques et culturels : Imprégnez-vous de l'histoire millénaire du Japon en visitant des sites emblématiques tels que le temple Senso-ji à Asakusa, le sanctuaire Meiji à Shibuya et le palais impérial au cœur de Tokyo. Découvrez l'élégance intemporelle des jardins japonais et des quartiers traditionnels comme Yanaka et Kagurazaka.
        
        🍣🍱 Dégustation de la délicieuse cuisine japonaise : Éveillez vos papilles avec une incroyable variété de plats japonais, des sushis et sashimis frais aux ramens fumants, en passant par les tempuras croustillants et les délices sucrés des wagashi. Explorez les marchés alimentaires animés et les restaurants étoilés pour une expérience culinaire inoubliable.
        
        🌸🏞️ Émerveillement de la nature et des jardins japonais : Échappez à l'agitation de la ville en vous promenant dans les jardins paisibles de Tokyo, tels que le jardin national Shinjuku Gyoen et le jardin Hama-rikyu au bord de l'eau. Admirez les cerisiers en fleurs au printemps et les érables rouges en automne pour une expérience visuelle époustouflante.
        
        🎭🎶 Divertissements et innovations technologiques : Plongez dans la culture pop japonaise en explorant les quartiers animés d'Akihabara et de Harajuku, où la technologie de pointe se mêle à la mode excentrique et à la culture des otakus. Découvrez les jeux d'arcade futuristes, les cafés à thème uniques et les boutiques de gadgets étonnants.
        
        🛍️🎁 Shopping et design avant-gardiste : Faites du shopping dans les grands magasins extravagants, les boutiques de créateurs branchées et les marchés aux puces vintage de Tokyo, où vous trouverez des trésors uniques, des produits de haute technologie et des souvenirs uniques à ramener chez vous.
        
        Votre séjour à Tokyo promet d'être une aventure inoubliable, où chaque instant est une découverte et chaque expérience est une merveille. Réservez dès maintenant et préparez-vous à vivre des moments magiques dans cette métropole fascinante du Japon ! 🇯🇵✨`,
        imageSrc: "/nav/asie/tokyo.jpg",
      },
      {
        name: "Seoul",
        slug: "seoul",
        price: "200",
        description: `🌟🏙️ Explorez l'éclat de Séoul : Un séjour captivant au cœur de la Corée du Sud ! 🍜🎎

        Plongez dans l'effervescence électrisante de Séoul, une métropole moderne imprégnée de tradition et d'innovation, où chaque coin de rue révèle une nouvelle surprise et chaque quartier a son propre charme distinct.
        
        🏨 Hébergement dans des hôtels modernes et confortables : Découvrez le confort et le luxe en séjournant dans des hôtels de renommée internationale situés au cœur de la ville, offrant un accès facile aux principales attractions et aux quartiers animés de Séoul.
        
        🏯🌸 Exploration des trésors historiques et culturels : Parcourez les palais magnifiques, les temples ancestraux et les quartiers traditionnels de Séoul pour découvrir son riche patrimoine culturel. Ne manquez pas de visiter le palais Gyeongbokgung, le temple Jogyesa et le village hanok de Bukchon pour une immersion dans l'histoire et la tradition coréennes.
        
        🍜🥢 Dégustation de la délicieuse cuisine coréenne : Éveillez vos papilles avec une incroyable variété de plats traditionnels coréens, des succulentes grillades de barbecue aux nouilles épicées, en passant par les savoureux bibimbaps et les délicieux banchans. Explorez les marchés de rue animés et les restaurants locaux pour une expérience gastronomique authentique.
        
        🌳🚶 Promenades dans les parcs et les jardins : Échappez à l'agitation de la ville en vous promenant dans les magnifiques parcs et jardins de Séoul, comme le parc Namsan et le parc olympique, où vous pourrez profiter de la nature luxuriante, des vues panoramiques et des sentiers de randonnée pittoresques.
        
        🎭🎶 Vie nocturne animée et divertissements culturels : Imprégnez-vous de l'énergie vibrante de la vie nocturne de Séoul en explorant ses bars branchés, ses clubs animés et ses salles de karaoké. Assistez à des spectacles de musique traditionnelle coréenne, des représentations de danse contemporaine et des concerts de K-pop pour une expérience culturelle inoubliable.
        
        🛍️🎁 Shopping et innovations technologiques : Faites du shopping dans les centres commerciaux ultramodernes, les marchés animés et les boutiques de design avant-gardiste de Séoul, où vous trouverez des produits de haute technologie, des articles de mode tendance et des souvenirs uniques à ramener chez vous.
        
        Votre séjour à Séoul promet d'être une aventure passionnante, où la fusion entre tradition et modernité crée une expérience unique au cœur de la Corée du Sud. Réservez dès maintenant et préparez-vous à vivre des moments inoubliables dans cette ville dynamique et captivante ! 🇰🇷✨`,
        imageSrc: "/nav/asie/seoul.jpg",
      },
    ],
  },
  {
    label: "Amérique",
    value: "amerique" as const,
    featured: [
      {
        name: "New-York",
        slug: "new-york",
        price: "200",
        description: `🗽🌆 Explorez la splendeur de New York : Un séjour inoubliable dans la ville qui ne dort jamais ! 🍎🎭

        Bienvenue dans la ville emblématique de New York, où l'énergie vibrante, la diversité culturelle et les icônes mondiales se rencontrent pour créer une expérience de voyage inoubliable. Plongez-vous dans l'effervescence de cette métropole cosmopolite, où chaque rue révèle une nouvelle surprise et chaque quartier a son propre caractère distinctif.
        
        🏨 Hébergement dans des hôtels emblématiques : Découvrez le luxe et le confort en séjournant dans des hôtels prestigieux situés au cœur de Manhattan, offrant des vues époustouflantes sur le skyline de la ville et un accès facile aux principales attractions. Des établissements historiques aux hôtels design ultramodernes, New York offre une gamme d'options d'hébergement pour tous les goûts et tous les budgets.
        
        🏙️🌃 Exploration des quartiers emblématiques : Parcourez les quartiers emblématiques de New York, de Times Square animé à Central Park verdoyant en passant par le chic Upper East Side et le bohème Greenwich Village. Imprégnez-vous de l'atmosphère unique de chaque quartier, découvrez des boutiques de créateurs, des galeries d'art inspirantes et des restaurants gastronomiques de renommée mondiale.
        
        🍎🍕 Dégustation de la délicieuse cuisine new-yorkaise : Éveillez vos papilles avec une incroyable variété de saveurs, des pizzas authentiques aux bagels délicieux, en passant par les steaks juteux et les desserts alléchants. Explorez les marchés alimentaires éclectiques, les restaurants ethniques et les célèbres food trucks pour une expérience gastronomique inoubliable à chaque bouchée.
        
        🎭🎶 Culture et divertissement : Imprégnez-vous de la scène artistique dynamique de New York en assistant à des spectacles de Broadway, des concerts de musique live et des expositions d'art contemporain dans les musées mondialement connus. Découvrez l'effervescence de la vie nocturne new-yorkaise avec ses bars branchés, ses clubs animés et ses soirées festives qui durent jusqu'au petit matin.
        
        🛍️🛒 Shopping et glamour urbain : Faites du shopping dans les boutiques de luxe de la Cinquième Avenue, les magasins branchés de SoHo et les marchés vintage de Brooklyn, où vous trouverez des marques de créateurs, des articles uniques et des souvenirs inoubliables à ramener chez vous.
        
        Votre séjour à New York promet d'être une aventure inoubliable, où la diversité culturelle et l'effervescence urbaine vous captiveront à chaque instant. Réservez dès maintenant et préparez-vous à vivre des moments mémorables dans la ville la plus emblématique du monde ! 🌟🗽`,
        imageSrc: "/nav/amerique/new-york.jpg",
      },
      {
        name: "Montréal",
        slug: "montreal",
        price: "200",
        description: `🍁🏙️ Découvrez l'effervescence de Montréal : Un séjour inoubliable au cœur du Québec ! 🥖🎭

        Bienvenue dans la métropole multiculturelle de Montréal, où l'histoire, la gastronomie et le dynamisme urbain se mêlent pour offrir une expérience de voyage authentique et enrichissante. Plongez-vous dans l'ambiance unique de cette ville cosmopolite, où chaque coin de rue raconte une histoire fascinante.
        
        🏨 Hébergement dans des établissements de charme : Découvrez le confort et le style en séjournant dans des hôtels élégants et accueillants situés au cœur de Montréal. Que vous préfériez le charme des hôtels historiques du Vieux-Montréal ou le luxe moderne du centre-ville, vous trouverez l'endroit idéal pour vous ressourcer après une journée d'exploration.
        
        🍁🎨 Exploration des quartiers emblématiques : Parcourez les quartiers emblématiques de Montréal, du Vieux-Port animé au quartier artistique du Plateau Mont-Royal en passant par le quartier historique du Mile End. Imprégnez-vous de l'atmosphère unique de chaque quartier, découvrez des boutiques pittoresques, des galeries d'art inspirantes et des cafés animés.
        
        🍽️🥂 Dégustation de la délicieuse cuisine montréalaise : Éveillez vos papilles avec une incroyable variété de saveurs, des classiques de la cuisine québécoise comme la poutine et la tourtière aux plats internationaux raffinés. Explorez les marchés alimentaires dynamiques, les restaurants branchés et les brasseries artisanales pour une expérience gastronomique inoubliable.
        
        🏞️🚴 Détente dans la nature urbaine : Échappez à l'agitation de la ville en explorant les espaces verts de Montréal, comme le magnifique parc du Mont-Royal et le paisible canal Lachine. Profitez de balades à vélo, de randonnées pédestres et de pique-niques relaxants dans ces oasis urbaines où la nature est à portée de main.
        
        🎭🎶 Culture et divertissement : Imprégnez-vous de la scène artistique dynamique de Montréal en assistant à des spectacles de théâtre, des concerts de musique live et des festivals culturels tout au long de l'année. Explorez les musées fascinants, les galeries d'art contemporain et les événements culturels uniques qui font de Montréal une ville bouillonnante d'énergie créative.
        
        🛍️🎁 Shopping et artisanat local : Faites du shopping dans les boutiques branchées, les marchés artisanaux et les magasins vintage de Montréal, où vous trouverez des trésors uniques, des souvenirs authentiques et des créations locales à ramener chez vous.
        
        Votre séjour à Montréal promet d'être une expérience inoubliable, où la diversité culturelle et la convivialité québécoise vous accueillent à bras ouverts. Réservez dès maintenant et préparez-vous à vivre des moments mémorables dans cette ville dynamique et accueillante du Canada ! 🇨🇦✨`,
        imageSrc: "/nav/amerique/montreal.jpg",
      },
      {
        name: "Mexico",
        slug: "mexico",
        price: "200",
        description: `🌵🏰 Explorez la vibrante Mexico : Un voyage fascinant au cœur de la culture mexicaine ! 🌮🎉

        Bienvenue dans la capitale animée du Mexique, où l'histoire ancienne, l'art coloré et la cuisine délicieuse se rencontrent pour créer une expérience de voyage inoubliable. Plongez dans l'énergie électrique de Mexico et découvrez les trésors cachés de cette ville cosmopolite.
        
        🏨 Hébergement dans des hôtels charmants et authentiques : Découvrez l'hospitalité chaleureuse du Mexique en séjournant dans des hôtels élégants situés au cœur de la ville ou dans des quartiers pittoresques, offrant un mélange unique de confort moderne et de charme traditionnel mexicain.
        
        🏰🎨 Exploration des trésors historiques et artistiques : Parcourez les siècles d'histoire mexicaine en visitant des sites emblématiques tels que le Zócalo, le Templo Mayor et le Palacio de Bellas Artes, qui témoignent de la grandeur et de la richesse culturelle du passé précolombien et colonial du Mexique. Imprégnez-vous de l'art coloré de Diego Rivera et Frida Kahlo dans les musées et galeries d'art de renommée mondiale.
        
        🌮🍹 Dégustation de la délicieuse cuisine mexicaine : Éveillez vos papilles avec une incroyable variété de plats mexicains, des tacos et tamales aux enchiladas et ceviches, en passant par les délices sucrés des churros et des paletas. Explorez les marchés alimentaires animés et les stands de rue pour une expérience culinaire authentique.
        
        🏞️🌵 Émerveillement devant la beauté naturelle et les sites naturels : Échappez à l'agitation de la ville en explorant les trésors naturels du Mexique, tels que les pyramides de Teotihuacán, les canaux de Xochimilco et les volcans entourant la ville. Plongez-vous dans la beauté sauvage de la nature mexicaine et découvrez des paysages époustouflants à chaque tournant.
        
        🎉💃 Divertissements et festivals culturels : Imprégnez-vous de la culture mexicaine en participant à des festivals traditionnels, des défilés colorés et des célébrations musicales. Découvrez la passion du mariachi, la grâce du ballet folklorique et l'énergie contagieuse des danses traditionnelles.
        
        🛍️🎁 Shopping et artisanat local : Faites du shopping dans les marchés artisanaux, les boutiques de design mexicain et les galeries d'art contemporain de Mexico, où vous trouverez des trésors uniques, des souvenirs authentiques et des œuvres d'art locales à ramener chez vous.
        
        Votre séjour à Mexico promet d'être une aventure inoubliable, où chaque moment est une découverte et chaque expérience est une merveille. Réservez dès maintenant et préparez-vous à vivre des moments magiques dans cette ville dynamique et colorée du Mexique ! 🇲🇽✨`,
        imageSrc: "/nav/amerique/mexico.jpg",
      },
    ],
  },
]
